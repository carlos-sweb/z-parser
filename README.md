...
- **`yield`/`await` are flag-scoped**: `Parser.yield_allowed`/`await_allowed` (set per-body by z-functions) gate `.yield_expr` (AssignmentExpression level, argument-less when a terminator or newline follows) and `.await_expr` (unary level; off, the contextual `await` stays an ordinary identifier). `async function` at expression position hands off to the function hook without advancing. Deferred: `yield*` delegation, async arrows.
