...
- **Functions/arrow functions**: need parameter-list + statement-body grammar; arrow functions additionally need a "cover grammar" (`(a, b)` parses as a possible grouped/sequence expression first, reinterpreted as parameters if `=>` follows) — real, self-contained work for its own phase, downstream of [z-statements](https://github.com/carlos-sweb/z-statements) since function bodies *are* statement lists.
